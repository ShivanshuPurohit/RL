...
def step(state, action):
    ...
    reward = -1
    if (action == ACTIONS[1] and i == 2 and 1 <= j <= 10) or (
        action == ACTIONS[3] and state == START):
        reward = -100
        next_state = START
...
# actionRewards[:, :, :] = -1.0
# actionRewards[2, 1:11, DOWN] = -100.0
# actionRewards[3, 0, RIGHT] = -100.0
...
#         destination = dict()
#         destination[UP] = [max(i - 1, 0), j]
#         destination[LEFT] = [i, max(j - 1, 0)]
#         destination[RIGHT] = [i, min(j + 1, WORLD_WIDTH - 1)]
#         if i == 2 and 1 <= j <= 10:
#             destination[DOWN] = START
#         else:
#             destinaion[DOWN] = [min(i + 1, WORLD_HEIGHT - 1), j]
#         actionDestination[-1].append(destinaion)
# actionDestination[3][0][RIGHT] = START
...
def print_optimal_policy(q_value):
    ...
    for i in range(0, WORLD_HEIGHT):
        ...
        for j in range(0, WORLD_WIDTH):
            ...
            if bestAction == ACTIONS[0]:
                optimal_policy[-1].append('U')
            elif bestAction == ACTIONS[1]:
                optimal_policy[-1].append('D')
            elif bestAction == ACTIONS[2]:
                optimal_policy[-1].append('L')
            elif bestAction == ACTIONS[3]:
